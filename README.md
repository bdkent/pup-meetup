...
src/sources/*.js           per-source parsers (.ics, jsonld/eventbrite, rss, instagram)
src/catalog.js  geocode.js  store.js  ingest.js   pipeline (catalog -> data/events/)
site/render.js             pure HTML rendering for every page type
site/build.js              generates the multi-page static site into site/dist/
...
Filtering is **navigation between pre-generated static pages**, not a client-side
app — so JS ships on the **index only** (the Leaflet map + a ~0.5 KB navigator).
Every other page is **zero-JS** static HTML; locations link out to Google Maps.

```
index.html                  all upcoming + map + breed/location pickers
breed/<breed>.html          one breed
metro/<metro>.html          one location
find/<breed>__<metro>.html  combined (the common case), only for pairs with events
org/<id>.html               an organizer's upcoming meetups + their source links
event/<id>.html (+ .ics)    one meetup, with add-to-calendar
```
